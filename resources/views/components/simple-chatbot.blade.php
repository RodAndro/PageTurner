...
function sendSimpleMessage(event) {
    ...
    .catch(error => {
        ...
        // Get book recommendations from database as fallback
        fetch('/api/v1/books')
            .then(response => response.json())
            .then(books => {
                const botResponseDiv = document.createElement('div');
                ...
                if (books.data && books.data.length > 0) {
                    responseContent = `<p style="font-size: 14px; color: #1f2937; margin: 0 0 12px 0;">Here are ${books.data.length} character-driven literary fiction books from our collection:</p>`;
                    ...
                    books.data.slice(0, 3).forEach((book, index) => {
                        responseContent += `
                            <div style="margin-bottom: 12px; padding: 8px; background: #f9fafb; border-left: 3px solid #16a34a; border-radius: 4px;">
                                ...
                                <p style="font-size: 12px; color: #059669; margin: 0;"><strong>Why you'd like it:</strong> Perfect for readers who enjoy deep character development and literary prose with strong emotional depth.</p>
                            </div>
                        `;
                    });
                } else {
                    responseContent = `<p style="font-size: 14px; color: #1f2937; margin: 0;">I'd love to help you find character-driven literary fiction! Let me check what we have in our collection. Could you tell me more about what specifically draws you to this genre? For example, are you looking for something with complex characters, beautiful writing, or particular themes?</p>`;
                }
            ...
            .catch(error => {
                ...
                botResponseDiv.innerHTML = `
                    ...
                    <div style="background: white; border-radius: 8px; padding: 12px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); max-width: 80%;">
                        <p style="font-size: 14px; color: #1f2937; margin: 0;">I'm having trouble accessing our book database right now, but I'd still love to help! Could you try refreshing the page or let me know what specific type of character-driven fiction interests you most?</p>
                    </div>
                `;
                messagesContainer.appendChild(botResponseDiv);
                messagesContainer.scrollTop = messagesContainer.scrollHeight;
